...
function massageOverrides(input) {
	var overrides = { views: input.views || {} }; // the output. ensure a `templates` hash
	var subObj;

	// iterate through all option override properties (except `templates`)
			...
			if (
				...
				$.each(val, function(subName, subVal) {
					...
					if (/^(month|week|day|default|basic(Week|Day)?|agenda(Week|Day)?)$/.test(subName)) {
						...
						overrides.views[subName][name] = subVal; // record the value in the `templates` object
					}
...
var Calendar = fc.Calendar = fc.CalendarBase = Class.extend({
	...
	getUnitViewSpec: function(unit) {
		...
		if ($.inArray(unit, intervalUnits) != -1) {
			...
			$.each(fc.views, function(viewType) { // all templates
